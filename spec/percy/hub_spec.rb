RSpec.describe Percy::Hub do
  ...
  describe 'global locks limit' do
    describe '#set_global_locks_limit' do
      ...
      it 'fails if given bad limit' do
        expect do
          hub.set_global_locks_limit(limit: nil)
        end.to raise_error(TypeError)
      end
    end

    describe '#get_global_locks_limit' do
      it 'gets the set limit or default' do
        expect(hub.get_global_locks_limit).to eq(10000)
        hub.set_global_locks_limit(limit: 5)
        expect(hub.get_global_locks_limit).to eq(5)
      end
    end
  end
  ...
  describe '#_enqueue_next_job' do
    ...
    it 'returns hit_global_lock_limit when global limit is hit' do
      ...
      hub.set_global_locks_limit(limit: 1)
      ...
      expect(hub._enqueue_next_job(build_id: 234, subscription_id: 345))
        .to eq('hit_global_lock_limit')
      expect(hub.redis.llen('jobs:runnable')).to eq(1)
      expect(hub.redis.zcount('global:locks:claimed', '-inf', '+inf')).to eq(1)

      # Release one of the global locks and enqueue another job.
      hub.release_job(job_id: first_job_id)
      expect(hub.redis.zcount('global:locks:claimed', '-inf', '+inf')).to eq(0)
    ...
    it 'returns hit_lock_limit when subscription lock limit is exceeded' do
      ...
      expect(hub._enqueue_next_job(build_id: 234, subscription_id: 345)).to eq(1)
      expect(hub.redis.llen('jobs:runnable')).to eq(2)
      expect(hub.redis.zcount('subscription:345:locks:claimed', '-inf', '+inf')).to eq(2)
      expect(hub.redis.zcount('global:locks:claimed', '-inf', '+inf')).to eq(2)
  ...
  describe '#worker_job_complete' do
    ...
    it 'removes the job from the worker:<id>:running queue' do
      expect(hub.redis.lrange("worker:#{worker_id}:running", 0, 10)).to eq(['1'])
      expect(hub.worker_job_complete(worker_id: worker_id)).to eq('1')
  ...
  describe '#release_job' do
    ...
    it 'records stats' do
      ...
      expect(hub.stats).to receive(:gauge)
        .once.with('hub.global.locks.limit', 10000).and_call_original
      expect(hub.stats).to receive(:gauge)
        .once.with('hub.global.locks.claimed', 0).and_call_original

      hub.release_job(job_id: job_id)
    end

    it 'returns false if no job exists to cleanup' do
      hub.insert_job(job_data: 'process_comparison:123', build_id: 234, subscription_id: 345)
      expect(hub.release_job(job_id: 1)).to eq(true)
      expect(hub.release_job(job_id: 1)).to eq(false)
    end
  end
  ...
  describe '#_record_global_locks_stats' do
    it 'records the number of workers online (0) and idle (0)' do
      expect(hub.stats).to receive(:gauge).once.with('hub.global.locks.limit', 10000)
      expect(hub.stats).to receive(:gauge).once.with('hub.global.locks.claimed', 0)
      hub._record_global_locks_stats
    end
  end
